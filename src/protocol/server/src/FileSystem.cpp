...
std::string truncateString(const std::string &str, size_t size) {
  if (str.length() <= size) {
    return str; // If the string is already shorter or equal to the desired
                // size, return the original string
  } else {
    return str.substr(0, size); // Extract a substring of the desired length
  }
}
...
FileSystem::FileSystem()
    : directory(new FileProperties[BLOCK_COUNT]()),
      FAT(new BLOCK_INDEX[BLOCK_COUNT]()), unit(new u8[STORAGE_VOLUME]()) {
  for (size_t i = 0; i < BLOCK_COUNT; ++i) {
    this->FAT[i] = UNUSED;
  }
  this->fillWithFile("./src/data.txt");
}
...
bool FileSystem::write(const std::string name, std::string &buffer,
  ...
  for (int i = 0; i < bufferSize; ++i) {
    ...
    // Write a character
    // LOG("Write: Writing character " << buffer[i] << "\n")
...
std::string FileSystem::read(const std::string name, size_t readSize) {
  ...
  for (; canContinue; i = this->FAT[i]) {
    canContinue =
        (i < BLOCK_COUNT) && (i != LAST_BLOCK) && (counter <= readSize);
    if (!canContinue) {
      break;
    }
...
void FileSystem::fillWithFile(std::string filename) {
  std::ifstream file("./src/data.txt");
  ...
  while (std::getline(file, line)) {
    ...
    const std::string userFileName = splitString(line, ",")[0];
    LOG("User file name: " << userFileName)
    FileProperties newUser(userFileName, "30-4-2023", "ADMIN");
...
void FileSystem::DumpToFile(std::string filename) {
...
void FileSystem::change2WriteMode(const std::string name) {
  DIRECTORY_INDEX file_index = this->search(name);
