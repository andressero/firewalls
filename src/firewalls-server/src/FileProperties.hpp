...
class FileProperties {
  ...
  public:
    ...
    bool operator==(const FileProperties& properties) const {
      ...
      const bool equalStartingBlock = this->startingBlock ==
        properties.startingBlock;
      const bool equalCursor = this->cursor == properties.cursor;
      const bool equalLastAccesedBlock = this->lastAccesedBlock ==
        properties.lastAccesedBlock;
      const bool equalDirectoryIndex = this->directoryIndex ==
        properties.directoryIndex;
    ...
    inline string getOwner() const {
      return this->owner;
    }

    inline BLOCK_POINTER getStartingBlock() const {
      return this->startingBlock;      
    }

    inline UNIT_POINTER getCursor() const {
      return this->cursor;
    }
    ...
    // Sets
    inline void setCursor(UNIT_POINTER cursor) {
      this->cursor = cursor;
    }
